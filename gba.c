...
void drawFullScreenImageDMA(const u16 *image) {
    ...
    DMA[3].cnt = DMA_SOURCE_INCREMENT|DMA_DESTINATION_INCREMENT|DMA_ON|WIDTH*HEIGHT;
}
...
void fillScreenDMA(volatile u16 color) {
    DMA[3].src = &color;
    DMA[3].dst = videoBuffer;
    DMA[3].cnt = DMA_SOURCE_FIXED|DMA_DESTINATION_INCREMENT|DMA_ON|WIDTH*HEIGHT;
    //did in lab, not given (could be buggy)
}
